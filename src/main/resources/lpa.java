...
public class lpa {
	static int Vertex=7;
	static int[] Lable_t=new int[Vertex];//???????????,t???
	static int[] Lable_t_1=new int[Vertex];//???????????,t-1???
	static int[] Importance_sorting=new int[Vertex];//??????????????
	static int[] Vertex_neighbour_lable=new int[Vertex];//?????????????
	static int[] Sum_degree=new int[Vertex];//?????????????
	//static int[][] Degree_array=new int[Vertex][2];//??????
	static int [][] Adjmartrix=new int[Vertex][Vertex];//?????????	
	/**********************
	 * ????????????????��?????
	 * ???????????????
	 * *************************/
	public static void main(String[] args) {
		...
		/**********************?????*************************/
		Initing(Edge_graph);	
		Ininting_Degree_Sorting(Edge_graph);//?????????????????????????
	...
	/**********************??????????????*************************/
	public static void Ininting_Degree_Sorting(int Edge[][]){//??????????��??????????????��??
